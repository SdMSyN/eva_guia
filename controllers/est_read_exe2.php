<?php
    ...
    if(!isset($_SESSION['exeRand'])){
        ...
        //print_r($arrIds);
        $arrIdsRand = array_rand($arrIds, 2);
        $arrRand = array();
    ...
    $reload = '../views/est_read_exe2.php?idNivel='.$idNivel.'&idUser='.$idUser;
    //aleatorio
    $idPregSess = $_SESSION['exeRand'][$page-1];
    ...
    while($rowGetInfo = $resGetInfo->fetch_assoc()){
        ...
        while($rowGetResp = $resGetResp->fetch_assoc()){
            ...
            $resps[] = array('id'=>$idResp, 'nombre'=>$nombreResp, 'archivo'=>$archivoResp);
        }
        //aleatorio
    ...
    echo json_encode(array("error"=>0, "dataPregs"=>$pregs, 'pags'=>$paginador, 'sql'=>$_SESSION['exeRand']));
